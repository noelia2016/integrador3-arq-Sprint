...
public interface CarreraRepo extends JpaRepository<Carrera, Long> {
	 ...
	 */
	@Query("SELECT NEW com.example.demo.dtos.EstudianteCarreraDTO(c , e, i, i.fecha as fech) FROM Inscripto i JOIN i.estudiante e JOIN i.carrera c")
	...
	// String consulta = "SELECT NEW dtos.EstudianteCarreraDTO(c , e, i, i.fecha as
	// fech) FROM Inscripto i JOIN i.estudiante e JOIN i.carrera c";
	// TypedQuery<EstudianteCarreraDTO> query =
	// RepositoryFactory.getEntity_manager().createQuery(consulta,
	// EstudianteCarreraDTO.class);
	// List<EstudianteCarreraDTO> ingresos = query.getResultList();

	// consulta = "SELECT NEW dtos.EstudianteCarreraDTO(c , e, i, (i.fecha + i.
	// antiguedad) as fech) FROM Inscripto i JOIN i.estudiante e JOIN i.carrera c
	// WHERE i.esEgresado = 1";
	// query = RepositoryFactory.getEntity_manager().createQuery(consulta,
	// EstudianteCarreraDTO.class);
